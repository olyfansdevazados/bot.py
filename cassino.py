...
async def cassinos(c: Client, m: Union[Message, CallbackQuery]):
    ...
    start_message = f"""<a href='https://images.unsplash.com/photo-1518895312237-a9e23508077d?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1184&q=80'>&#8204</a> 🎮Gosta de apostas e ganhar dinheiro no bot ao mesmo tempo? Está no lugar certo.

Aposte seu saldo e ganhe dinheiro
...
async def stardices(c: Client, m: Message):
    ...
    if balance < price:
            ...
            "Você não possui saldo para realizar esta aposta. Por favor, adicione saldo no menu principal.",show_alert=True,)
...
<b>Você não tirou 6 🎲, foram descontados R$ 6,00 da sua conta</b>"""
           await om.reply_text(text, quote=True)						
...
<b>Você não acertou o alvo 🎯, foram descontados R$ 7,00 da sua conta<b/>"""
           await om.reply_text(text, quote=True)
...
<b>Você não acertou o alvo 🎯, foram descontados R$ 8,00 da sua conta</b>"""
           await om.reply_text(text, quote=True)						
...
<b>Você não acertou a cesta 🏀, foram descontados R$ 4,00 da sua conta</b>"""
           await om.reply_text(text, quote=True)						
...
async def startpar(c: Client, m: Message):
    ...
    if (om.dice.value%2) == 0:
           ...
           text = f"Você ganhou R$ 5,00 de saldo, por apostar em PAR ✌️"
           await om.reply_text(text, quote=True)
...
<b>Você perdeu, deu IMPAR ☝️, foram descontados R$ 5,00 da sua conta</b>"""
           await om.reply_text(text, quote=True)		        		
...
<b>Você perdeu, deu PAR ✌️, foram descontados R$ 5,00 da sua conta</b>"""
           await om.reply_text(text, quote=True)		        	       	
...
<b>Você não deu STRIKE 🎳 , foram descontados R$ 8,00 da sua conta</b>"""
           await om.reply_text(text, quote=True)						
