...
async def chk(c: Client, m: Message):
    ...
    if balance < price:
        await m.reply_text(
            "❌ Parece que você não pode usar o checker, porquê você não tem saldo suficiente, recarregue no menu principal. ❌")
            
    elif not cctest:
        await m.reply_text(
            "<b>⚠️ Insira uma cc para testar ⚠️</b>")
    else:
    	await m.reply_text(
