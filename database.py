...
CREATE TABLE IF NOT EXISTS bot_config(
    lara_name TEXT DEFAULT 'Só pix automático',                 -- Nome da lara
    lara_key TEXT DEFAULT 'Só pix automático',                                        -- Chave pix da lara
    ...
    support_user TEXT DEFAULT '@Spaceofcc',                               -- Username de atendimento
    channel_user TEXT DEFAULT '@Spaceofcc',                                    -- Canal de notícias
...
CREATE TABLE IF NOT EXISTS log_gate (gate TEXT, retorno TEXT, hora_log TEXT);



CREATE TABLE IF NOT EXISTS custom_gate(
    url TEXT DEFAULT 'https://gatexemplo.com/api.php?key1234&card=',                           -- url da gate
    resultlive TEXT DEFAULT 'true',                           -- resposta da gate ao testar a cc
    resultdie TEXT DEFAULT 'false'                              -- retorno de die
);

-- Inicializa a configuração com os valores padrão acima --
-- As configurações podem ser alteradas via painel posteriormente --
INSERT OR IGNORE INTO custom_gate(ROWID) values(0);
