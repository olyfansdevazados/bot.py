...
async def buy_mixes(c: Client, m: CallbackQuery):
    ...
    if not levels_list:
        return await m.answer(
            "NÃ£o hÃ¡ Mixes disponÃ­veis no momento, tente novamente mais tarde.",
            show_alert=True,
        )
    ...
    organ.append([InlineKeyboardButton(text="ðŸ”™ Voltar", callback_data="comprar_cc")])
    kb = InlineKeyboardMarkup(inline_keyboard=organ)

    await m.edit_message_text(
        f"""<b>ðŸ“¦ Comprar Mix</b>
...
async def buy_mix(c: Client, m: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="ðŸ”™ Voltar", callback_data="comprar_cc"),
            ],
        ]
    )
