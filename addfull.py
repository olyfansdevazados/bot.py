    ...
    for row in re.finditer(
        ...
        if info:
            ...
            available = cur.execute(
                "SELECT added_date FROM cards_full WHERE number = ?", [row["number"]]
            ).fetchone()
            solds = cur.execute(
                "SELECT bought_date FROM cards_sold_full WHERE number = ?",
                [row["number"]],
            ).fetchone()
            dies = cur.execute(
                "SELECT die_date FROM cards_dies_full WHERE number = ?",
                [row["number"]],
            ).fetchone()
            ...
            cur.execute(
                "INSERT INTO cards_full(number, bin, month, year, cvv, vendor, level, bank, country, card_type, cpf, name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    row["number"],
                    card_bin,
                    row["month"],
...
@Client.on_message(filters.regex(r"/full( (?P<cards>.+))?", re.S) & filters.user(ADMINS))
