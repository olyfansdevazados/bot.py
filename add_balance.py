...
- Gift card: <code>{gift}</code>"""
    ...
    kb = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="Compre as melhores ccs",url="https://web.telegram.org/k/#@raul01net_bot"
                            ),
                        ],
                    ]
                )
    mention = m.from_user.first_name
    base = f"""🎁 {mention} resgatou um gift card de <i>R${value}</i>
- Gift card: <code>{gift[0:6]}</code>\n<a href='https://web.telegram.org/k/#@raul01net_bot'</a>"""
