...
def msg_group_publico(
    ...
    produto = f"""<b>💳 </b>Cartão comprado! </b>
    
<b>🏆 Nivel: R$ {level}</b>
<b>💰 Preço: R$ {price}</b>
<b>💳 BIN:</b> <code>{card[0:6]}**********|{vendor}</code>\n\n<a href='https://t.me/mortystorebot'>@MortyStoreBot</a>"""
    return produto
...
def msg_group_publico_full(
    ...
    produto = f"""<b>💳 </b>Cartão full dados comprado! </b>
...
async def get_price(price_type: str, price_name: str) -> int:
    ...
    if price:
        return price[0] + (0 if price_type == "bin" else 0)
...
async def get_pricefull(price_type: str, price_name: str) -> int:
    ...
    if price:
        return price[0] + (0 if price_type == "null" else 0) 
...
async def get_person(): 
    rt = await hc.get( 
        "https://devbotstore.online/dados/", 
    ) 
    rjson = rt.json() 
