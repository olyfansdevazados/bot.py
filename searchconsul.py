...
async def buy_off_consul(c: Client, m: CallbackQuery):
    ...
    if not selected_cc:
        return await m.answer("Sem consul disponiveis para este nivel.", show_alert=True)
    ...
    if balance < price:
        return await m.answer(
            f"Você não possui saldo suficiente para esse item R$ {price}. Por favor, faça uma transferência.",
            show_alert=True,
        )
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
    ...
    await c.send_message(GRUPO_PUB, adm_msg)
